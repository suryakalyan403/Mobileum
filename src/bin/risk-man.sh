...
then
    ...
    wait_pod_ready "${deploymentName}" "${productNamespace}" "${type}" "1800s"
	...
	then
		...
		wait_pod_ready "${pres_deploymentName}" "${productNamespace}" "rafm-satellite" "1800s"
	...
	then	
		...
		wait_pod_ready "caddy" "${productNamespace}" "" "1800s"
